<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="staffName" label="客服"></el-table-column>
...
import { countLiveAgentRecord, queryLiveAgentRecord, getTopicTranscript } from '@/api/stats';
  ...
  methods: {
    ...
    query () {
      ...
      if (this.lastQueryParams !== json) {
        ...
        queryLiveAgentRecord(this.start, this.end, this.pageNo - 1, this.pageSize)
          .then(list => {
            this.loading = false;
            list.forEach(e => {
              ...
              const lastUpdate = moment(e.lastUpdatedAt);
              e.duration = lastUpdate.diff(start, 'seconds');
              ...
              e.rate = {
                verySatisfied: '非常满意',
                satisfied: '满意',
                dissatisfiedWithStaff: '对客服代表不满意',
                dissatisfiedWithBusinessRule: '对业务规定不满意'
              }[e.rate];
            });
            this.tableData = list;
          });
    ...
    count () {
      ...
      countLiveAgentRecord(start, end)
        .then(count => {
          this.total = count;
        });
    },
    ...
    async queryNoPage () {
      this.loading = true;
      const list = await queryLiveAgentRecord(this.start, this.end, 0, this.total);
      this.loading = false;
    ...
    async exportToExcel () {
      ...
      ws['!cols'] = [
        {width: 10}, {width: 25}, {width: 20}, {width: 10},
        {width: 15}, {width: 20}, {width: 15}, {width: 15},
        {width: 25}
      ];
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, '人工客服请求记录明细');
      const wbout = XLSX.write(wb, {type: 'array', bookType: 'xlsx'});
      window.saveAs(new Blob([wbout], {type: 'application/octet-stream'}), '人工客服请求记录明细.xlsx');
    }
  }
};
</script>
