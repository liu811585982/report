<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="topicName" label="话题名"></el-table-column>
...
import { countBotRecord, queryBotRecord, getTopicTranscript } from '@/api/stats';
  ...
  methods: {
    ...
    query () {
      ...
      if (this.lastQueryParams !== json) {
        ...
        queryBotRecord(this.start, this.end, this.pageNo - 1, this.pageSize)
          .then(list => {
            this.loading = false;
            list.forEach(e => {
    ...
    async queryNoPage () {
      this.loading = true;
      const list = await queryBotRecord(this.start, this.end, 0, this.total);
      this.loading = false;
    ...
    async exportToExcel () {
      ...
      ws['!cols'] = [
        {width: 10}, {width: 25}, {width: 20}, {width: 20},
        {width: 15}, {width: 20}, {width: 20}, {width: 15},
        {width: 25}
      ];
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, '机器人话题记录明细');
      const wbout = XLSX.write(wb, {type: 'array', bookType: 'xlsx'});
      window.saveAs(new Blob([wbout], {type: 'application/octet-stream'}), '机器人话题记录明细.xlsx');
    }
  }
};
</script>
